...
public class LoginThree {
    ...
    public void loginScript() throws Exception {
      ...
      //  driver.findElement(By.id(loginone)).click();
       ...
       this.element(this.byStr(zero,loginone)).click();
        Thread.sleep(3000);
           //输入邮箱或者手机号
          ...
          WebElement user = this.element(this.byStr(one,WebElementemail));
           user.isDisplayed();
           //输入密码
           WebElement password = this.element(this.byStr(two,WebElementpassword));
           password.isDisplayed();
           //WebElement loginButton = this.element(By.className("btn-red"));
        //登录按钮
           WebElement loginButton = this.element(this.byStr(three,WebElementloginButton));
           ...
           Thread.sleep(2000);
           //鼠标悬浮在个人信息
           WebElement header = this.element(this.byStr(four,header_info));
           header.isDisplayed();
        Thread.sleep(1000);
           Actions actions = new Actions(driver);
           actions.moveToElement(header).perform();
           String userInfo = this.element(this.byStr(five,user_nicheng)).getText();
           System.out.println(userInfo);
           if(userInfo.equals("qq_big大山mount_03375711")){
               System.out.println("登录成功login33!");

           }else {
               System.out.println("登录失败login333S!");
           }
    }
    /***
     * by封装定位方法
    ...
    public static void main(String[] args) throws Exception {
        LoginThree loginThree = new LoginThree();
        loginThree.InitDriver();
        loginThree.loginScript();
